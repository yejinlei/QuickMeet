...
for i in $(seq 1 75); do
  ...
  echo "   [${i}/75] healthy(${#GOOD}w): ${GOOD:-none} | unhealthy: ${BAD:-none} | 非 running: ${DOWN:-none}"
  if [ "$BAD" = "" ] && [ -z "${DOWN// /}" ]; then
    N=$(echo "$GOOD" | sed '/^$/d' | wc -w)
    [ "$N" -ge 5 ] && { ALL_OK=1; break; }
  fi
...
if [ "$ALL_OK" = 1 ]; then
  echo
  echo "🟢 五容器全部 healthy，耗时 $(elapsed)s"
  echo "   信令入口：http://127.0.0.1:8081/healthz"
  exit 0
else
  echo
  echo "🔴 未在 5 分钟内全部 healthy，耗时 $(elapsed)s"
