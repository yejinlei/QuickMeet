...
依赖全部服务端阶段（第一~三阶段）；AI 依赖走本地硅基流动接口

## 交付内容

- 全服务 Dockerfile 与 docker-compose.yml 编排：媒体服务、信令、NATS、AI 网关、存储
- 严格兼容 docker-compose 1.29.2，禁止 docker-compose-plugin 特性
- 单条 `docker-compose up -d` 拉起全套服务，端口 8080，适配内网 192.168.0.0/24

## 本 Issue 验收标准

1. `docker-compose up -d` 单命令成功拉起全部服务，无配置报错
2. compose 文件在 docker-compose 1.29.2 下 `config` 校验通过
3. 服务健康检查与重启策略完备，容器日志可查
4. 提供部署文档：前置条件、步骤、回滚方式
